...
{
    public partial class frmAddVendorPayment : Form
    {

        public int glbItemID = -1;
        public DataTable glbdt;
        public frmAddVendorPayment()
        {
            InitializeComponent();
        }
        ...
        private void btnsave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    //DALAccess.ExecuteNonQuery("insert into vendorpayment (vendorid,amount,amounttopay,balance,Createdat,createdby,paiddate) values ("+VendorID+","+AmountPaid+","+txtamounttopay.Text+","+Balance+",'"+DateTime.Now+ "',"+LoggedInUser.UserID+",'" + DateTime.Now + "') ");
                    DALAccess.ExecuteNonQuery("insert into vendorpayment (vendorid,amount,amounttopay,balance,Createdat,createdby,paiddate) values (" + VendorID + "," + AmountPaid + ",0,0,'" + DateTime.Now + "'," + LoggedInUser.UserID + ",'" + DateTime.Now + "') ");
                    txtamountpaid.Text = "0";
        ...
        public void BindGrid(string name)
        {
            try
            {
                ...
                DataTable dt = DALAccess.ExecuteDataTable(@"select vendor.Name as VendorName,vendorpayment.ID,vendorpayment.amount,vendorpayment.balance,
                vendorpayment.createdat,vendor.id as vendorid,vendorpayment.amounttopay
                    from (vendor inner join vendorpayment on vendor.ID=vendorpayment.vendorid)
                    where " + name+@"
                    ");
        ...
        private void ddlvendor_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                string query = @"select 
                            sum(vendorpayment.amounttopay) as amounttopay
                            ,sum(vendorpayment.amounttopay) as Price
                            ,sum(vendorpayment.balance) as balance
                            ,sum(vendorpayment.amount) as amount
                            ,sum(vendorpayment.amount) as Total
                            ,sum((vendorpayment.amounttopay - vendorpayment.amount)) as balance2
                            from vendorpayment inner join vendor on vendor.id=vendorpayment.vendorid where vendorpayment.vendorid=" + vendorid;
